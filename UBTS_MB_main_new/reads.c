...
void read_udpCommand(){
	...
	if (i++ >= 0x00ff) {
		...
		command_exec(checkPermission(commands_decoder(&FIFO_udpChRx, &COMMAND), UDP_CH));
		i = 0;
	}
}
void read_sendGps() {
...
void read_mainCommand() {
	...
	if (i++ >= 0x01ff) {
		...
		command_exec(checkPermission(commands_decoder(&FIFO_mainChRx, &COMMAND), MAIN_CH));
		//memcpy(FIFO_debugChTx.data, COMMAND.buffer, COMMAND.length);
		//FIFO_debugChTx.head += COMMAND.length;
		//w5200_sendDataFifo(DEBUG_CH, &FIFO_debugChTx);
		i=0;
	}
}
...
void read_eeprom() {
	...
	animePause = read_eeprByte(PAUSE_EEPR) << 8 | read_eeprByte(PAUSE_EEPR + 1);
	if (animePause == 0xffff) animePause = DEFAULT_ANIME_PAUSE;	
}
